...
async def stats(user_id:str,request:Request,user:dict=Depends(get_current_user)):
    ...
    auto_reconciled=tiers["1"]+tiers["2"]+tiers["3"]
    classes={k:sum(r.get("classification")==k for r in rows) for k in ["Paid","Partial","Unpaid","Exception"]}
    ai_resolved=sum(1 for r in rows if r.get("match_tier")==4 and r.get("classification")!="Exception")
    unresolved_exceptions=classes["Exception"]
    ...
    def is_today(x,field): return isinstance(x.get(field),datetime) and x[field]>=today_start

    total_outstanding=sum(max((inv.get("amount") or 0)-(inv.get("amount_paid") or 0),0) for inv in invoices)
    total_collected=sum(p.get("amount") or 0 for p in payments if p.get("status")=="captured")
    total_settled=sum(s.get("amount") or 0 for s in settlements)

    matched_payment_ids={pid for r in rows for pid in r.get("payment_ids",[])}
    unmatched_payments=[p for p in payments if p.get("razorpay_payment_id") not in matched_payment_ids]
    pending_payments=sum(1 for p in payments if p.get("status") in ("created","authorized"))

    # Duplicate/suspicious: captured payments sharing the same (email, amount) more than once.
    seen={}; duplicate_payments=0
    for p in payments:
        if p.get("status")!="captured": continue
        key=(p.get("email"),p.get("amount"))
        seen[key]=seen.get(key,0)+1
    duplicate_payments=sum(count-1 for count in seen.values() if count>1)

    matched_settlement_ids={sid for r in rows for sid in r.get("settlement_ids",[])}
    settlement_exceptions=sum(1 for s in settlements if s.get("razorpay_settlement_id") not in matched_settlement_ids)

    deltas=[abs(r.get("settlement_delta")) for r in rows if r.get("settlement_delta") is not None]
    payment_settlement_difference=sum(deltas)
    confidences=[r.get("confidence") for r in rows if isinstance(r.get("confidence"),(int,float))]
    avg_confidence=round(sum(confidences)/len(confidences)*100,1) if confidences else 0

    return {
        "counts":{
            "total_invoices":len(invoices),"total_payments":len(payments),"total_settlements":len(settlements),
            "fully_paid":classes["Paid"],"partially_paid":classes["Partial"],"unpaid":classes["Unpaid"],
        },
        "amounts":{
            "total_outstanding":total_outstanding,"total_collected":total_collected,"total_settled":total_settled,
            "payment_settlement_difference":payment_settlement_difference,
        },
        "reconciliation":{
            "total_matched_records":total,"match_rate":round(auto_reconciled/total*100,2) if total else 0,
            "auto_reconciled":auto_reconciled,"ai_resolved":ai_resolved,"unresolved_exceptions":unresolved_exceptions,
            "avg_confidence":avg_confidence,"by_tier":tiers,
        },
        "risk":{
            "duplicate_payments":duplicate_payments,"unmatched_payments":len(unmatched_payments),
            "pending_payments":pending_payments,"settlement_exceptions":settlement_exceptions,
        },
        "throughput":{
            "records_processed":len(invoices)+len(payments)+len(settlements),
            "last_reconciled_at":max((r.get("reconciled_at") for r in rows if r.get("reconciled_at")),default=None),
        },
        "classification":classes,
        "activity":{
            "invoices":[period(invoices,"issued_at",1),period(invoices,"issued_at",7),period(invoices,"issued_at",30)],
            "payments":[period(payments,"captured_at",1),period(payments,"captured_at",7),period(payments,"captured_at",30)],
        },
        "today":{
            "invoices":[i for i in invoices if is_today(i,"issued_at")][:20],
            "payments":[p for p in payments if is_today(p,"captured_at")][:20],
        },
        "exceptions":exceptions[:20],
        "recent":{"invoices":invoices[:8],"payments":payments[:8],"settlements":settlements[:8]},
    }
...
async def search(
    ...
    # Each collection uses a different field for "when it happened".
