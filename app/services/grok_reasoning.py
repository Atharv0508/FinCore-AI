...
class GrokReasoningService:
    ...
    async def answer_exception_question(self, question: str, evidence: dict[str, Any]) -> dict[str, Any]:
        ...
        system_prompt=(
            ...
            "structured evidence provided under 'evidence' - never invent invoice numbers, amounts, dates, "
            ...
            "Format every answer as plain text with a REAL newline character (\\n) between each numbered "
            "point below - each point must start on its own line, never all on one line, like this:\n"
            "1. One-line verdict.\n"
            ...
            "Do not use Markdown symbols like # or **, and do not join points with spaces or semicolons instead of newlines.\n"
            "Return valid JSON only in exactly this shape: {\"answer\":\"...\",\"cited_exception_ids\":[]}."
        )
        payload={"model":self.settings.groq_model,"temperature":0,"messages":[{"role":"system","content":system_prompt},{"role":"user","content":json.dumps({"question":question,"evidence":evidence},default=_json_default)}],"response_format":{"type":"json_object"}}
        try:
